<script lang="ts">
	import { CookingPot, Edit, Trash } from 'lucide-svelte';

    export let name: string;
    export let servings: number;
    export let totalCalories: number;
    export let totalProteins: number;
    export let totalCarbs: number;
    export let totalFat: number;
...
<div class="card {isNew ? 'highlight' : ''} recipe-card">
	<!-- Parte izquierda: nombre -->
	<div class="recipe-card__main">
		<div class="recipe-card__icon">
			<CookingPot style="width: 1rem; height: 1rem;" />
		</div>

		<h3 class="recipe-card__name">{name}</h3>
	</div>

	<!-- Parte derecha: info nutricional -->
	<div class="recipe-card__nutrients">
		<div class="nutrient">
			<span class="nutrient-label">Raciones</span>
			<span class="nutrient-value">{servings}</span>
		</div>
		<div class="nutrient">
	...
	<div class="recipe-card__actions">
		<button type="button" on:click={onEdit} class="btn-icon" aria-label="Editar receta" style="width: 2rem; height: 2rem; padding: 0;">
			<Edit style="width: 1rem; height: 1rem;" />
		</button>

		<button type="button" on:click={onDelete} class="btn-icon-warning" aria-label="Eliminar receta" style="width: 2rem; height: 2rem; padding: 0;">
